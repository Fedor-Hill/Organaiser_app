...
class SheduleMobilePage extends StatelessWidget {
  const SheduleMobilePage(
      {super.key, required this.sheduleBloc, required this.sheduleModel});
  ...
  Widget build(BuildContext context) {
    ...
    final String date = DateFormat.MMMMEEEEd(appLocal.localeName)
        .format(sheduleModel.dayModel[0].date)
        .toString();
      ...
      children: [
        Expanded(
          child: Padding(
              ...
              child: SheduleList(sheduleModel: sheduleModel, func: onClick,)),
        ),
        Padding(
...
class SheduleList extends StatelessWidget {
  const SheduleList({super.key, required this.sheduleModel, required this.func});
  ...
  Widget build(BuildContext context) {
    ...
    if (sheduleModel.dayModel[0].lessons.isEmpty) {
      return Center(
      ...
      return CustomScrollView(slivers: [
        SliverAppBar(
            title: Text(
                appLocal.lesson_count(sheduleModel.dayModel[0].lessons.length),
                style: theme.textTheme.headlineSmall),
            leading: DateTime.now().day != sheduleModel.dayModel[0].date.day ? IconButton(
            ...
            bottom: null),
        SliverList.builder(
          itemCount: sheduleModel.dayModel[0].lessons.length,
          itemBuilder: (context, index) => SheduleTile(
              index: index,
              lessonSheduleModel: sheduleModel.dayModel[0].lessons[index]),
        )
      ]);
    }
  }
